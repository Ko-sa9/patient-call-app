...
const AdminPage = () => {
    ...
    const initialMasterFormData = { patientId: '', lastName: '', firstName: '', furigana: '', day: '月水金', cool: '1' };
    ...
    const initialDailyFormData = { lastName: '', firstName: '', furigana: '' };
    ...
    const [editDailyFormData, setEditDailyFormData] = useState({ lastName: '', firstName: '', furigana: '', });
    ...
    const handleOpenMasterModal = (patient = null) => {
        ...
        setFormError('');
        if (patient) {
            setMasterFormData({ patientId: patient.patientId || '', lastName: patient.lastName || '', firstName: patient.firstName || '', furigana: patient.furigana || '', day: patient.day, cool: patient.cool });
    ...
    const handleAddTempFromMaster = async (patient) => {
        try {
            await addDoc(dailyPatientsCollectionRef(selectedCool), { name: patient.name, furigana: patient.furigana || '', status: '治療中', isTemporary: true, masterPatientId: patient.patientId, masterDocId: patient.id, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    ...
    const handleAddDailySubmit = async (e) => {
        e.preventDefault();
        if (!dailyFormData.lastName || !dailyFormData.firstName) { setFormError('必須項目をすべて入力してください。'); return; }
        try {
            await addDoc(dailyPatientsCollectionRef(selectedCool), { name: `${dailyFormData.lastName} ${dailyFormData.firstName}`.trim(), furigana: dailyFormData.furigana, status: '治療中', isTemporary: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
            setAddDailyModalOpen(false);
    ...
    const handleOpenEditDailyModal = (patient) => {
        ...
        setEditDailyFormData({ lastName: nameParts[0] || '', firstName: nameParts.slice(1).join(' ') || '', furigana: patient.furigana || '' });
    ...
    const handleEditDailySubmit = async (e) => {
        e.preventDefault();
        if (!editDailyFormData.lastName || !editDailyFormData.firstName) { setFormError('必須項目をすべて入力してください。'); return; }
        if (!editingDailyPatient) return;
        try {
            await updateDoc(doc(dailyPatientsCollectionRef(selectedCool), editingDailyPatient.id), { name: `${editDailyFormData.lastName} ${editDailyFormData.firstName}`.trim(), furigana: editDailyFormData.furigana, updatedAt: serverTimestamp() });
        ...
        const loadAction = async () => {
            ...
            try {
                ...
                masterSnapshot.forEach(patientDoc => {
                    ...
                    batch.set(doc(dailyPatientsCollectionRef(selectedCool)), { name: `${patientData.lastName || ''} ${patientData.firstName || ''}`.trim() || patientData.name || '', furigana: patientData.furigana || '', status: '治療中', masterPatientId: patientData.patientId || null, masterDocId: patientDoc.id, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
                });
                await batch.commit();
...
const MonitorPage = () => {
    ...
    const treatmentPatients = allPatients.filter(p => p.status === '治療中').sort((a, b) => (a.furigana || '').localeCompare(b.furigana || '', 'ja'));
...
const QrScannerModal = ({ onClose, onScanSuccess }) => {
    ...
    useEffect(() => {
        ...
        const config = { fps: 5, qrbox: { width: 250, height: 250 }, formatsToScan: [Html5QrcodeSupportedFormats.QR_CODE, Html5QrcodeSupportedFormats.CODE_128, Html5QrcodeSupportedFormats.EAN_13, Html5QrcodeSupportedFormats.CODABAR] };
...
const CompactQrScanner = ({ onScanSuccess }) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const config = { fps: 5, qrbox: { width: 110, height: 110 }, formatsToScan: [Html5QrcodeSupportedFormats.QR_CODE, Html5QrcodeSupportedFormats.CODE_128, Html5QrcodeSupportedFormats.EAN_13, Html5QrcodeSupportedFormats.CODABAR] };
