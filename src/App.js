...
import { Html5Qrcode, Html5QrcodeSupportedFormats } from 'html5-qrcode'; // QRコード・バーコード読み取り用ライブラリ
import * as wanakana from 'wanakana'; // 日本語のローマ字・かな変換ライブラリ

// --- Firebase Configuration ---
// Firebaseプロジェクトの設定情報。環境変数からAPIキーを読み込むことで、セキュリティを確保しています。
...
// --- Initialize Firebase ---
// Firebaseの各サービスを初期化し、アプリ全体で利用できるようにします。
const app = initializeApp(firebaseConfig);
const auth = getAuth(app); // 認証サービス
const db = getFirestore(app); // Firestoreデータベース
const functions = getFunctions(app, 'us-central1'); // Cloud Functions（リージョン指定）

// --- Helper Components & Functions (共通で使う便利機能) ---

/**
 * 今日の日付を 'YYYY-MM-DD' 形式の文字列で取得します。
 * タイムゾーンを日本標準時(JST)に調整しています。
...
/**
 * 日付文字列から、Firestoreでクエリするための曜日文字列('月水金' or '火木土')を返します。
 * @param {string} dateString - 'YYYY-MM-DD' 形式の日付文字列
 * @returns {string|null} '月水金', '火木土', または対象外(日曜日)の場合は null
 */
const getDayQueryString = (dateString) => {
    const date = new Date(dateString);
    const dayIndex = date.getDay(); // 0:日, 1:月, ... 6:土
    if ([1, 3, 5].includes(dayIndex)) return '月水金'; // 月・水・金
    if ([2, 4, 6].includes(dayIndex)) return '火木土'; // 火・木・土
    return null; // 日曜日
};

// データの読み込み中に表示するスピナーコンポーネント
...
// アプリ内で使用する汎用的なモーダル（ポップアップウィンドウ）コンポーネント
...
// 「はい/いいえ」をユーザーに確認するためのモーダルコンポーネント
...
// --- App Context for Shared State (アプリ全体で共有する情報) ---
const AppContext = createContext(); // Contextを作成
const FACILITIES = ["本院透析室", "坂田透析棟", "じんクリニック", "木更津クリニック"]; // 施設リスト

// --- Custom Hooks (特定の機能に関するロジックをまとめたもの) ---

/**
 * Firestoreから指定されたクールの日次患者リストをリアルタイムで取得するカスタムフック。
 * グローバル設定（施設、日付、クール）が変更されると自動でデータを再取得します。
 */
const useDailyList = () => {
    const { selectedFacility, selectedDate, selectedCool } = useContext(AppContext); // グローバル設定を取得
    const [dailyList, setDailyList] = useState([]); // 日次リストの状態
    const [loading, setLoading] = useState(true); // 読み込み状態

    useEffect(() => {
        // グローバル設定が未選択の場合は何もしない
...
            setDailyList(fetchedDailyPatients); // 取得したデータでstateを更新
            setLoading(false);
        }, (err) => {
...
        return () => unsubscribe();
    }, [selectedFacility, selectedDate, selectedCool]); // 依存配列: これらの値が変わると再実行

    return { dailyList, loading };
};

/**
 * Firestoreから指定された日の全クール(1, 2, 3)の患者リストを取得し、
 * 一つのリストに結合して提供するカスタムフック。
 * モニター画面やスタッフ画面などで使用します。
 */
const useAllDayPatients = () => {
    const { selectedFacility, selectedDate } = useContext(AppContext); // グローバル設定を取得
    const [allPatients, setAllPatients] = useState([]); // 全患者リストの状態
    const [loading, setLoading] = useState(true); // 読み込み状態
...
        const cools = ['1', '2', '3']; // 対象のクール
        const unsubscribes = []; // 監視解除用の関数を格納する配列
        let patientData = { '1': [], '2': [], '3': [] }; // 各クールのデータを一時的に保持
        let loadedFlags = { '1': false, '2': false, '3': false }; // 各クールの初回読み込み完了フラグ

        // 各クールのデータを結合して、allPatientsステートを更新する関数
        const updateCombinedList = () => {
            const combined = Object.values(patientData).flat();
            setAllPatients(combined);
        };
        
        // 全てのクールが読み込み完了したかチェックし、完了ならloadingをfalseにする関数
...
                updateCombinedList(); // データを結合して更新
                    // 初回読み込みが完了したらフラグを立て、全体の読み込み完了をチェック
                if (!loadedFlags[cool]) {
...
                }
            });
            unsubscribes.push(unsubscribe); // 監視解除用関数を保存
        });

        // コンポーネントがアンマウントされる時に全ての監視を解除
        return () => {
            unsubscribes.forEach(unsub => unsub());
        };
    }, [selectedFacility, selectedDate]); // 依存配列
...
/**
 * Firestore上の患者ドキュメントのステータスを更新する非同期関数。
 * @param {string} facility - 施設名
 * @param {string} date - 日付
 * @param {string} cool - クール
 * @param {string} patientId - 患者ドキュメントID
 * @param {string} newStatus - 新しいステータス ('治療中', '呼出中', '退出済')
 */
...
            status: newStatus,
            updatedAt: serverTimestamp() // 更新日時をサーバータイムスタンプで記録
        });
    } catch (error) {
...
// 患者のステータスに応じて色が変わるバッジを表示するコンポーネント
...
    
    // ふりがな自動入力のためのstateとref
    const [furiganaParts, setFuriganaParts] = useState({ last: '', first: '' });
    const isFuriganaManuallyEdited = useRef(false); // ふりがなを手動編集したかどうかのフラグ
    const [formError, setFormError] = useState(''); // フォームのエラーメッセージ
    const [masterSearchTerm, setMasterSearchTerm] = useState(''); // マスタリストの検索キーワード
    
    // 臨時患者追加モーダル関連のstate
...
    // --- データ取得 (Effects) ---
    // 選択中の施設が変わった時に、患者マスタをFirestoreからリアルタイム取得する
    useEffect(() => {
...
                const data = doc.data();
                // 姓名フィールド(lastName, firstName)と、古い形式のnameフィールドの両方に対応
                let lastName = data.lastName || '';
                let firstName = data.firstName || '';
                let name = '';
                if (lastName || firstName) { name = `${lastName} ${firstName}`.trim(); } 
                else if (data.name) { // 古いデータ形式へのフォールバック
                    name = data.name;
...
    
    // --- ふりがな自動入力ロジック ---
    // 姓・名のふりがな部分が変更されたら、結合してフォームデータに反映する
    useEffect(() => {
        if (isFuriganaManuallyEdited.current) return; // 手動編集時は自動入力を無効化
...
        };
        try { 
            if (editingMasterPatient) { // 編集モードの場合
...
    const handleCloseAddDailyModal = () => setAddDailyModalOpen(false);
    ...
    // 臨時患者追加（手動）フォームの入力ハンドラ
    const handleDailyFormChange = (e) => {
        const { name, value } = e.target;
        setFormError('');

        // ふりがなのロジックはマスタフォームと同様
        if (name === 'furigana') {
...
                status: '治療中', isTemporary: true, // 臨時患者フラグ
                masterPatientId: patient.patientId, // どのマスタ患者から来たかの情報
...
    };
    
    // 当日リストの患者情報編集モーダルを開く処理
    const handleOpenEditDailyModal = (patient) => {
        // nameフィールドを姓と名に分割してフォームにセット
...
        const dayQuery = getDayQueryString(selectedDate); // '月水金' or '火木土'
...
                
                // writeBatchを使って複数の書き込みを一度に（アトミックに）実行
...
        };
        // もし既に当日リストにデータがあれば、上書き確認のモーダルを表示
...
    };
    ...
    // 当日リストの患者情報を、最新のマスタ情報に同期（上書き）する処理
...
        try {
            await updateDoc(dailyPatientDocRef, updates); // 変更点のみを更新
        } catch (error) {
...
                                // ソート処理: 「退出済」を一番下に、それ以外はベッド番号の昇順で並び替え
...
                                // マスタ情報と当日リスト情報に差異があるか、かつマスタの方が新しいかチェック
...
                                    masterPatients.filter(p => { // 検索フィルター
...
                                               (p.furigana && p.furigana.toLowerCase().includes(term));
                                    })
                                    .sort((a, b) => { // 曜日 > クール > ベッド番号 の優先順位でソート
...
// --- 2. Monitor Page (モニター表示画面) ---
/**
 * 公開用のモニター画面コンポーネント。
 * 「お呼び出し済み」と「治療中」の患者をリアルタイムで表示し、
 * 新たに呼び出された患者の情報を音声で読み上げます。
 */
const MonitorPage = () => {
    const { allPatients, loading } = useAllDayPatients(); // その日の全患者データを取得
    // ステータスに応じて患者をフィルタリングし、ベッド番号でソート
...
    
    // --- 音声読み上げ機能のロジック ---
    const prevCallingPatientIdsRef = useRef(new Set()); // 前回の呼び出し中患者IDリスト
    const [isSpeaking, setIsSpeaking] = useState(false); // 現在、音声再生中かどうかのフラグ
    const speechQueueRef = useRef([]); // 読み上げるべき患者のキュー
    
    // --- 音声停止機能のために追加 ---
...
    const nextSpeechTimerRef = useRef(null); // 次の再生までのタイマーを管理
    ...
    /**
     * 再生キューから次の患者を取り出し、音声合成して再生する関数。
     * 再帰的に呼び出され、キューが空になるまで処理を続けます。
     */
    const speakNextInQueue = useCallback(() => {
        // 既存のタイマーがあればクリア（予期せぬ二重実行を防ぐ）
...
        
        const functionUrl = "https://synthesizespeech-dewqhzsp5a-uc.a.run.app"; // 音声合成用のCloud FunctionのURL

        // 読み上げテキストが空の場合はスキップ
        if (!textToSpeak || textToSpeak.trim() === "") {
            nextSpeechTimerRef.current = setTimeout(speakNextInQueue, 1000); // 1秒後に次のキューへ
            return;
        }
...
    }, [callingPatients, isSpeaking, speakNextInQueue]); // 依存配列
    
...
// --- 3. Staff Page (スタッフ用操作画面) ---
/**
 * スタッフが手動で患者の呼び出し操作を行うための画面コンポーネント。
 * QRコード/バーコードスキャンによる呼び出しも可能です。
 */
const StaffPage = () => {
    const { allPatients, loading } = useAllDayPatients(); // その日の全患者データを取得
    const { selectedFacility, selectedDate } = useContext(AppContext);
    
    const [isScannerOpen, setScannerOpen] = useState(false); // スキャナーモーダルの開閉状態

    // 操作対象となる患者（治療中または呼出中）をフィルタリングし、ベッド番号でソート
...
        .sort((a, b) => a.bed.localeCompare(b.bed, undefined, {numeric: true}));
    ...
    /**
     * QRコード/バーコードスキャンが成功したときに呼び出されるコールバック関数。
     * @param {string} decodedText - スキャンで読み取られたテキスト（患者ID）
     * @returns {{success: boolean, message: string}} 処理結果
     */
    const handleScanSuccess = useCallback((decodedText) => {
        let result;
        // 読み取ったIDを持ち、かつステータスが「治療中」の患者を探す
        const patientToCall = allPatients.find(p => p.masterPatientId === decodedText && p.status === '治療中');

        if (patientToCall) {
            // 対象が見つかったら、ステータスを「呼出中」に更新
...
/**
 * QRコード/バーコードを読み取るためのモーダルコンポーネント。
 * html5-qrcodeライブラリを使用しています。
 * @param {object} props - コンポーネントのプロパティ
 * @param {function} props.onClose - モーダルを閉じるための関数
 * @param {function} props.onScanSuccess - スキャン成功時に呼び出されるコールバック関数
 */
const QrScannerModal = ({ onClose, onScanSuccess }) => {
    const [scanResult, setScanResult] = useState(null); // スキャン結果メッセージの状態
    const isProcessingRef = useRef(false); // スキャン後の処理中フラグ（連続スキャン防止）
    
    // onScanSuccess関数が再生成されても最新のものを参照できるようにrefに保持
...
    // コンポーネントのマウント時に利用可能なカメラを取得
    useEffect(() => {
        Html5Qrcode.getCameras().then(devices => {
            if (devices && devices.length) {
                setCameras(devices);
                // カメラが未選択の場合、背面カメラ(back)を優先的に選択する
                if (!selectedCameraId) {
...
    // スキャナーの起動と停止を管理するEffect
    useEffect(() => {
        if (!selectedCameraId) return; // カメラが選択されるまで何もしない

        const html5QrCode = new Html5Qrcode('qr-reader-container');
        
        // スキャン成功時のコールバック
        const qrCodeSuccessCallback = (decodedText, decodedResult) => {
            if (isProcessingRef.current) return; // 処理中の場合は重複実行を防ぐ
            isProcessingRef.current = true;
            const result = onScanSuccessRef.current(decodedText); // 親コンポーネントの処理を実行
...
        // スキャナーの設定 (QRコードと主要なバーコードを読み取り対象に)
        const config = { 
            fps: 10, 
...
            }
        };
    }, [selectedCameraId]); // 選択カメラが変わったら再起動

    // カメラを切り替えるハンドラ
    const handleCameraSwitch = () => {
        if (cameras.length < 2) return;
        const currentIndex = cameras.findIndex(c => c.id === selectedCameraId);
        const nextIndex = (currentIndex + 1) % cameras.length; // 次のカメラのインデックスを計算
...
// --- Global Controls & Layout (共通レイアウト) ---

/**
 * 施設・日付・クールを選択するためのグローバルコントロールパネル。
 * @param {object} props - プロパティ
 * @param {boolean} props.hideCoolSelector - クール選択を非表示にするかどうか
 */
...
// アプリ全体の共通レイアウトコンポーネント（ヘッダー、メインコンテンツ、フッター）
...
// --- Role-based Views (役割ごとの画面) ---

/**
 * スタッフ用のビュー。管理、スタッフ、モニターの各ページをタブで切り替えられるようにします。
 */
const StaffView = ({ user, onGoBack }) => {
    const [currentPage, setCurrentPage] = useState('admin'); // 現在表示中のページの状態
    // モニターとスタッフ画面ではクール選択を非表示にする
...
    
    // currentPageの値に応じて表示するページコンポーネントを切り替える
    const renderPage = () => {
        switch (currentPage) {
...
/**
 * 公開用（送迎担当者など）のビュー。DriverPageのみを表示します。
 */
...
// --- Login / Role Selection (ログインと役割選択フロー) ---
...
// アプリ起動時の最初の画面。役割（スタッフ用/公開用）を選択します。
...
// --- Main App (アプリケーションのメインコンポーネント) ---
export default function App() {
    // --- State管理 ---
    const [user, setUser] = useState(null); // Firebase認証ユーザー情報
    const [authReady, setAuthReady] = useState(false); // 認証処理が完了したかどうかのフラグ
    // 画面遷移を管理するstate。'login' -> 'password' -> 'facilitySelection' -> ('staff' or 'public') のように遷移する
...
    // --- Effects ---
    // アプリ起動時に一度だけ実行され、Firebaseの匿名認証を行う
    useEffect(() => {
...
        if (role === 'staff') {
            setViewMode('password'); // スタッフの場合はパスワード認証画面へ
        } else {
            setViewMode('facilitySelection'); // 公開用の場合は施設選択画面へ
        }
    };

    // パスワード認証成功時の処理
    const handlePasswordSuccess = () => {
        setViewMode('facilitySelection'); // 施設選択画面へ
    };

    // 施設選択画面での処理
    const handleFacilitySelect = (facility) => {
        setSelectedFacility(facility);
        setViewMode(selectedRole); // 選択されていた役割の画面（'staff' or 'public'）へ遷移
    };
    
    // 「戻る」ボタンで役割選択画面に戻る処理
    const handleGoBack = () => {
